...
LocPredictor::LocPredictor(int width, int height){
    ROI = cv::Rect(0, 0, width - 1, height - 1);
    imageSize = cv::Rect(0, 0, width - 1, height - 1);
    ROISize = 0;
}

bool LocPredictor::isSecondMeas(){
    return secondMeas >= 2;
}

cv::Rect LocPredictor::ROIPrediction(float timeStamp){
    if (secondMeas < 2){
        ROI = imageSize;
    }
    else{
...
coord2D LocPredictor::pixelPosPredictor(float timeStamp){
    coord2D predPos;
...
float LocPredictor::depthPred(float timeStamp){
    if (secondMeas == 0){
        return 0.8;
    }
...
void LocPredictor::updateROIPredictor(coord2D currPos, float timeStamp, int currRadius){
...
cv::Rect LocPredictor::createROI(cv::Vec3f ballCircle, float error){
...
float LocPredictor::radiusPred(coord2D ballLocDepth, const struct rs2_intrinsics * intrin){
    float point[3];
...
// void LocPredictor::findEdge(float pixel[2], float changeX, float changeY, float point[3]){
//     point[0] += changeX;
//     point[1] += changeY;
