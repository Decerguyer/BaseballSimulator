...
Tracker::Tracker(int width, int height, struct rs2_intrinsics intrinsics, ThresholdFilter &threshFilter)
...
coord2D Tracker::findBallFromDepth(ImageData &imgData){
...
coord2D Tracker::findBallFromIR(ImageData &imgData, cv::Vec3f ballCircleDepth, coord2D ballCoordDepth){
 ...
 std::vector<std::vector<float>> Tracker::convertTo3D(std::vector<coord2D> coord2DVec, struct rs2_intrinsics intrinsics){
    ...
    for (int i = 0; i < coord2DVec.size(); i++){
        //if (coord2DVec[i].depth){
            ...
            float point[3];
            rs2_deproject_pixel_to_point(point, &intrinsics, pixel, coord2DVec[i].depth);
...
cv::Vec3f Tracker::depthVisHoughCircle(ImageData &imgData, float error){
...
cv::Vec3f Tracker::irHoughCircle(ImageData &imgData, float radius, float error){
    std::vector<cv::Vec3f> coords;
...
cv::Mat Tracker::cropFromPred(cv::Mat fullSizeMat, float timeStamp){
    cv::Rect ROI = locPred.ROIPrediction(timeStamp);
    return fullSizeMat(ROI);
}

cv::Mat Tracker::cropIR(ImageData &imgData, cv::Vec3f ballCircle, float error){
    return imgData.irMat(locPred.createROI(ballCircle, error));
}

cv::Point Tracker::ROIOffset(cv::Mat croppedImg){
    cv::Point offset;
    cv::Size wholesize;
...
float Tracker::findAveDepth(ImageData &imgData, cv::Vec3f circleCenter){
    float sum = 0;
    int numMeas = 0;
    for (int i = 0; i < 3; i++){
        for (int k = 0; k < 3; k++){
